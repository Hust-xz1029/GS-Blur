...
class Demosaic(nn.Module):
    ...
    def __init__(self):
        ...
        # [5,5] => rot90 => [1, 1, 5, 5]
        self.GR_GB_pt = torch.tensor(np.rot90(GR_GB, k=2).copy(), dtype=torch.float32, device='cuda')
        ...
        self.Rg_RB_Bg_BR_pt = torch.tensor(np.rot90(Rg_RB_Bg_BR, k=2).copy(), dtype=torch.float32, device='cuda')

        Rg_BR_Bg_RB = np.transpose(Rg_RB_Bg_BR)
        self.Rg_BR_Bg_RB_pt = torch.tensor(np.rot90(Rg_BR_Bg_RB, k=2).copy(), dtype=torch.float32, device='cuda')
        ...
        self.Rb_BB_Br_RR_pt = torch.tensor(np.rot90(Rb_BB_Br_RR, k=2).copy(), dtype=torch.float32, device='cuda')
